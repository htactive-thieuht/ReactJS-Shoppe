...
function Content(props) {
    ...
    const fetchData = async (keySearch) => {
        let ref = await database.ref('/products');
        ref.on('value', snapshot => {
            var state = snapshot.val();
            if (state) {
                const objectList = Object.keys(state).map(key => ({ ...state[key], id: key }))
                const filterData = objectList.filter(item => removeAccents3(item.name) == keySearch);
    ...
    const removeAccents3 = (str) => {
        const strMap = str.trim()
        return strMap.normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '')
